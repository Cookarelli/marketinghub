...
export function MaterialsEditor({value,onChange,available,onPendingChange,referenceOnly=false}:{value:Materials;onChange:(patch:Materials)=>void;available:Asset[];onPendingChange:(pending:boolean)=>void;referenceOnly?:boolean}) {
 ...
 <label className="field"><span>Upload a supporting file</span><Input type="file" accept={supportedAssetTypes.join(',')} disabled={uploading||!!file} onChange={e=>{const selected=e.target.files?.[0];if(selected){ticket.current=undefined;setFile(selected);void upload(selected);}e.target.value='';}}/></label>
