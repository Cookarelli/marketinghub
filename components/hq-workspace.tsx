...
type Action=(command:Record<string,unknown>)=>Promise<{id?:string}|null>;
type Workspace={records:WorkspaceRecord[];context:HqContext};
type Asset={id:string;data:{name:string}};
export const money=(cents:number)=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}).format(cents/100);
function dateLabel(value:string){return value?calendarDay(value)+' · '+calendarTime(value):'Not set';}
function Field({label,children}:{label:string;children:ReactNode}){return <label className="field"><span>{label}</span>{children}</label>;}
function Choice({label,value,onChange,options,disabled=false}:{label:string;value:string;onChange:(v:string)=>void;options:Record<string,string>;disabled?:boolean}){return <Field label={label}><select aria-label={label} value={value} onChange={e=>onChange(e.target.value)} disabled={disabled}>{Object.entries(options).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></Field>;}
function Person({label,value,onChange,staff,disabled=false}:{label:string;value:string;onChange:(v:string)=>void;staff:Staff[];disabled?:boolean}) {return <Choice label={label} value={value} onChange={onChange} disabled={disabled} options={{'':'Choose a person',...Object.fromEntries(staff.map(s=>[s.id,s.name])),...(value&&!staff.some(s=>s.id===value)?{[value]:value+' (inactive; reassign)'}:{})}}/>;}
...
function Missing({items}:{items:string[]}) {return items.length?<div className="notice"><strong>Needs attention</strong><ul>{items.map(x=><li key={x}>{x}</li>)}</ul></div>:<p className="muted">Required information is complete.</p>;}
function Attachments({assets,references,onChange,available}:{assets:string[];references:string[];onChange:(v:{assets:string[];references:string[]})=>void;available:Asset[]}) {
  return <details className="hq-details"><summary>Assets and reference links</summary><div className="campaign-fields">
    <p><Link href="/assets">Upload or manage files in Assets</Link></p>
    <fieldset className="hq-checks"><legend>Attach saved assets</legend>{available.length?available.map(a=><label key={a.id}><input type="checkbox" checked={assets.includes(a.id)} onChange={e=>onChange({references,assets:e.target.checked?[...assets,a.id]:assets.filter(x=>x!==a.id)})}/>{a.data.name}</label>):<p className="muted">No saved assets yet.</p>}</fieldset>
    <Field label="Reference links (HTTPS, one per line)"><Textarea rows={3} value={references.join('\n')} onChange={e=>onChange({assets,references:e.target.value.split('\n')})}/></Field>
  </div></details>;
}
function ResourceLinks({assets,references,available}:{assets:string[];references:string[];available:Asset[]}) {return <div className="hq-resource-links">{assets.map(id=><a href={'/api/assets/'+encodeURIComponent(id)} key={id} target="_blank" rel="noreferrer">{available.find(a=>a.id===id)?.data.name||'Saved asset'}</a>)}{references.filter(x=>/^https:\/\//.test(x)).map((url,i)=><a key={i} href={url} target="_blank" rel="noreferrer">Reference {i+1}: {url}</a>)}</div>;}
async function json<T>(url:string,options?:RequestInit):Promise<T> {const r=await fetch(url,{cache:'no-store',...options});const data=await r.json();if(!r.ok)throw new Error(data.error||'Could not load the workspace.');return data;}
...
export function HqWorkspace({view='list',id}:{view?:'list'|'project'|'deliverable';id?:string}) {
  ...
  return <div className="hq-workspace">
    ...
    {view==='list'&&<>
      <div className="section-title"><div><h2>Projects</h2><p className="muted">Scope the work, assign production and review each destination.</p></div><div className="button-row"><Button onClick={()=>setCreate('project')}>New project</Button><Button variant="outline" onClick={()=>setCreate('deliverable')}>New standalone work</Button></div></div>
      {create==='project'&&<ProjectForm initial={{...blankProject,owner:c.staffId}} context={c} assets={assets} busy={busy} onSave={createRecord} onCancel={()=>setCreate(null)}/>}
      ...
      <section className="panel"><h2>All deliverables</h2><DeliverableList records={deliverables} projects={projects} name={name}/></section>
      <LegacyAdoption records={records} context={c} act={act} busy={busy}/>
      {c.admin&&<details className="panel hq-details"><summary>Budget approval permissions</summary><p>Grant this separately from project ownership. Only these people can establish or change approved budgets.</p><div className="hq-checks">{c.staff.map(s=><label key={s.id}><input type="checkbox" checked={s.budgetApprover} disabled={busy} onChange={e=>void act({action:'permission',staffId:s.id,enabled:e.target.checked})}/>{s.name}</label>)}</div></details>}
    </>}
    {view==='project'&&(project?<>
      <section className="panel"><div className="section-title"><div><p className="eyebrow">{projectTypes[project.data.type]} · {projectStatuses[project.data.status]}</p><h2>{project.data.title}</h2><p>Owner: {name(project.data.owner)} · Members: {project.data.members.map(name).join(', ')||'None yet'}</p></div><span className="tag">{deliverables.filter(d=>d.data.projectId===project.id).length} deliverables</span></div><p className="hq-preserve-text">{project.data.brief||'Brief not yet recorded.'}</p><p>{project.data.type==='weekly_auction'?'Auction: '+dateLabel(project.data.auctionOpensAt)+' → '+dateLabel(project.data.auctionClosesAt):['event','product_release'].includes(project.data.type)?(project.data.type==='event'?'Event: ':'Release: ')+dateLabel(project.data.eventAt):''}</p><Missing items={projectMissing(project.data)}/><ResourceLinks assets={project.data.assets} references={project.data.references} available={assets}/></section>
      ...
      <BudgetPanel project={project} context={c} act={act} busy={busy} name={name}/>
      <section className="panel"><div className="section-title"><h2>Deliverables</h2>{eligibleProjects.some(p=>p.id===project.id)&&<Button onClick={()=>setCreate('deliverable')}>New deliverable</Button>}</div>
...
function ProjectList({projects,deliverables,name}:{projects:HqRecord<Project>[];deliverables:HqRecord<Deliverable>[];name:(id:string)=>string}) {
  ...
  return <section><div className="two-fields"><Field label="Find a project"><Input type="search" value={search} onChange={e=>setSearch(e.target.value)}/></Field><Choice label="Project status" value={filter} onChange={setFilter} options={{all:'All projects',...projectStatuses}}/></div><div className="hq-project-grid">{shown.map(p=><article key={p.id} className="panel"><p className="eyebrow">{projectTypes[p.data.type]} · {projectStatuses[p.data.status]}</p><h3><Link href={'/projects/'+p.id}>{p.data.title}</Link></h3><p>{name(p.data.owner)} · {deliverables.filter(d=>d.data.projectId===p.id).length} deliverables</p><p className="muted">{p.data.budget?money(p.data.budget.amountCents)+' approved':'Budget not approved'} · {projectMissing(p.data).length} missing items</p><Link href={'/projects/'+p.id}>Open project →</Link></article>)}</div>{!shown.length&&<p className="notice">No projects match. Create a project or adopt a saved campaign below.</p>}</section>;
}
function DeliverableList({records,projects,name}:{records:HqRecord<Deliverable>[];projects:HqRecord<Project>[];name:(id:string)=>string}) {
  ...
  return <><Choice label="Filter work" value={filter} onChange={setFilter} options={{all:'All work',standalone:'Standalone work',...productionStatuses}}/><ul className="hq-deliverables">{shown.map(({id,data:d})=><li key={id}><div><Link href={'/projects/work/'+id}>{d.title}</Link><p className="muted">{name(d.owner)} · Production: {dateLabel(d.productionDue)}</p><p className="muted">{d.projectId?projects.find(p=>p.id===d.projectId)?.data.title:'Standalone'}{d.blocked?' · Blocked: '+d.blockedReason:''}</p></div><div><strong>{productionStatuses[d.status]}</strong>{d.publishing&&<p className="muted">{Object.values(d.publications).filter(p=>p.status==='published').length}/{d.platforms.length} destinations published</p>}</div></li>)}</ul>{!shown.length&&<p className="muted">No deliverables in this view.</p>}</>;
}
...
function ProjectForm({initial,record,context:c,assets,busy,onSave,onCancel}:{initial:ProjectInput;record?:HqRecord<Project>;context:HqContext;assets:Asset[];busy:boolean;onSave:(command:Record<string,unknown>)=>Promise<void>;onCancel?:()=>void}) {
  ...
  return <form className="hq-form" onSubmit={e=>{e.preventDefault();void onSave({action:'save-project',id,version:record?.data.version||0,data:{...data,references:data.references.filter(x=>x.trim())}});}}>
    {!record&&<h3>New project</h3>}<fieldset disabled={busy} className="campaign-fields">
      ...
      <Attachments assets={data.assets} references={data.references} onChange={update} available={assets}/>
      ...
      <p className="muted">Saving changes requires renewed approval for pending publishing work.</p><div className="button-row"><Button type="submit">{busy?'Saving…':record?'Save project':'Create project'}</Button>{onCancel&&<Button type="button" variant="outline" onClick={onCancel}>Cancel</Button>}</div>
    </fieldset>
  </form>;
}
...
function DeliverableForm({initial,record,project,context:c,projects,assets,busy,onSave,onCancel}:{initial:DeliverableInput;record?:HqRecord<Deliverable>;project?:Project;context:HqContext;projects:HqRecord<Project>[];assets:Asset[];busy:boolean;onSave:(command:Record<string,unknown>)=>Promise<void>;onCancel?:()=>void}) {
  ...
  return <form className="hq-form" onSubmit={e=>{e.preventDefault();void onSave({action:'save-deliverable',id,version:record?.data.version||0,data:{...data,references:data.references.filter(x=>x.trim())}});}}>
    ...
    <fieldset disabled={busy||locked} className="campaign-fields">
      ...
      <label className="check-field"><input type="checkbox" checked={data.publishing} onChange={e=>update({publishing:e.target.checked,platforms:e.target.checked?['facebook','instagram']:[]})}/><span>This work will be published</span></label>
      {data.publishing&&<>
      ...
      <p className="muted">Changes after approval return this work to Needs review.</p><div className="button-row"><Button type="submit">{busy?'Saving…':record?'Save deliverable':'Create deliverable'}</Button>{onCancel&&<Button type="button" variant="outline" onClick={onCancel}>Cancel</Button>}</div>
    </fieldset>
  </form>;
}
...
function ProductionPanel({record,project,context:c,act,busy,name}:{record:HqRecord<Deliverable>;project?:Project;context:HqContext;act:Action;busy:boolean;name:(id:string)=>string}) {
  ...
  const next:Record<Deliverable['status'],Deliverable['status'][]>= {to_do:['in_progress'],in_progress:['to_do','needs_review'],needs_review:['in_progress',...(canApprove?['ready' as const]:[])],ready:['needs_review',...(d.publishing?[]:['done' as const])],done:['in_progress']};
  return <section className="panel"><h2>Production · {productionStatuses[d.status]}</h2><p>Deadline: {dateLabel(d.productionDue)}</p>{d.publishing&&<p>Intended publication: {dateLabel(d.publishAt)}</p>}{d.approval&&<p className="muted">{approvalCurrent(d,project)?'Approved':'Prior approval — review again'} by {name(d.approval.by)} · {recordedTime(d.approval.at)}</p>}
    {canWork(d,project,c)&&<div className="button-row">{next[d.status].map(status=><Button key={status} variant={status==='ready'?'default':'outline'} disabled={busy} onClick={()=>void act({action:'production',id:record.id,version:d.version,status})}>{status==='ready'?'Approve · Ready':status==='needs_review'&&d.status==='ready'?'Request another review':'Move to '+productionStatuses[status]}</Button>)}{d.status==='ready'&&!approvalCurrent(d,project)&&canApprove&&<Button disabled={busy} onClick={()=>void act({action:'production',id:record.id,version:d.version,status:'ready'})}>Renew approval</Button>}</div>}
    {d.status==='needs_review'&&!canApprove&&<p className="muted">Waiting for {name(approver)} to review.</p>}
  </section>;
}
function PublicationForm({record,project,platform,context:c,act,busy,name}:{record:HqRecord<Deliverable>;project?:Project;platform:string;context:HqContext;act:Action;busy:boolean;name:(id:string)=>string}) {
  ...
  return <article className="hq-publication"><h3>{platform}</h3><p className="tag">{p.status}</p>{p.scheduledFor&&<p>Scheduled for {dateLabel(p.scheduledFor)}<br/><span className="muted">Confirmed by {name(p.scheduledBy||'')}</span></p>}
    {p.status==='published'?<><p>Published {dateLabel(p.publishedAt||'')}<br/><span className="muted">Recorded by {name(p.recordedBy||'')}</span></p>{p.liveUrl?<a href={p.liveUrl} target="_blank" rel="noreferrer">Open live publication →</a>:<p>No live URL: {p.unavailableReason}</p>}</>:eligible?<form className="hq-form" onSubmit={e=>{e.preventDefault();void act({action:'publication',id:record.id,version:d.version,platform,status,time,url,unavailableReason:reason,confirmed});}}>
      ...
      </fieldset></form>:<p className="muted">An assigned team member records publishing confirmations.</p>}
  </article>;
}
...
function LegacyAdoption({records,context:c,act,busy}:{records:WorkspaceRecord[];context:HqContext;act:Action;busy:boolean}) {
  ...
  return <details className="panel hq-details"><summary>Adopt existing campaigns and calendar posts</summary><p>Move work into HQ explicitly. Original records remain preserved as read-only history. Historical approvals and publishing labels require fresh confirmation.</p><p className="muted">Recurring templates and editorial calendar copies stay in their current editors. Their data and attached media remain available.</p><ul className="hq-deliverables">{[...campaigns.map(r=>({...r,kind:'campaign',title:r.data.name})),...posts.map(r=>({...r,kind:'post',title:r.data.title}))].map(r=><li key={r.kind+r.id}><span>{r.title}</span><Button variant="outline" disabled={busy||(!c.admin&&r.data.owner!==c.staffId)} onClick={async()=>{const result=await act({action:r.kind==='campaign'?'adopt-campaign':'adopt-post',id:r.id});if(result?.id)router.push(r.kind==='campaign'?'/projects/'+result.id:'/projects/work/'+result.id);}}>Adopt {r.kind}</Button></li>)}</ul>{!campaigns.length&&!posts.length&&<p>No eligible legacy work remains.</p>}</details>;
}

type Comment={id:string;actor:string;body:string;created_at:string};
type Activity={id:string;actor:string;action:string;created_at:string;snapshot:{version?:number;title?:string;approval?:{by:string};budget?:{amountCents:number}}};
function Discussion({id,kind,context:c,act,busy}:{id:string;kind:'project'|'deliverable';context:HqContext;act:Action;busy:boolean}) {
  const [comments,setComments]=useState<Comment[]>([]),[activity,setActivity]=useState<Activity[]>([]),[next,setNext]=useState<number|null>(null),[error,setError]=useState(''),[body,setBody]=useState(''),[loading,setLoading]=useState(true),[refresh,setRefresh]=useState(0);
  ...
  return <section className="panel"><h2>Comments and activity</h2>{error&&<p role="alert">{error} <Button variant="outline" onClick={()=>setRefresh(n=>n+1)}>Retry history</Button></p>}
    <form className="hq-form" onSubmit={async e=>{e.preventDefault();if(await act({action:'comment',id,kind,commentId:commentId.current,body})){setBody('');commentId.current=clientId();setRefresh(n=>n+1);}}}><Field label="Add an internal comment"><Textarea required maxLength={5000} value={body} onChange={e=>{setBody(e.target.value);commentId.current=clientId();}}/></Field><Button type="submit" disabled={busy}>Post comment</Button></form>
    <ul className="hq-discussion">{comments.map(x=><li key={x.id}><strong>{name(x.actor)}</strong><time>{recordedTime(x.created_at)}</time><p className="hq-preserve-text">{x.body}</p></li>)}</ul>
    <details className="hq-details" open><summary>Activity history</summary><ul className="hq-discussion">{activity.map(x=><li key={x.id}><strong>{name(x.actor)}</strong><time>{recordedTime(x.created_at)}</time><p>{x.action.replaceAll('-',' ').replaceAll('_',' ')}{x.snapshot.title?' · '+x.snapshot.title:''}{x.snapshot.version?' · version '+x.snapshot.version:''}</p></li>)}</ul></details>
