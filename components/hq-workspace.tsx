...
type Activity={id:string;actor:string;action:string;created_at:string;snapshot:{version?:number;title?:string;approval?:{by:string};review?:{decision:'approve'|'changes';reviewedVersion:number};decision?:{reason:string};budget?:{amountCents:number}}};
export function Discussion({id,kind,context:c,act,busy}:{id:string;kind:'project'|'deliverable'|'request';context:HqContext;act:Action;busy:boolean}) {
  ...
  return <section className="panel"><h2>Comments and activity</h2>{error&&<p role="alert">{error} <Button variant="outline" onClick={()=>setRefresh(n=>n+1)}>Retry history</Button></p>}
    ...
    <details className="hq-details" open><summary>Activity history</summary><ul className="hq-discussion">{activity.map(x=><li key={x.id}><strong>{name(x.actor)}</strong><time>{recordedTime(x.created_at)}</time><p>{x.action.replaceAll('-',' ').replaceAll('_',' ')}{x.snapshot.title?' · '+x.snapshot.title:''}{x.snapshot.version?' · version '+x.snapshot.version:''}{x.action==='review'&&x.snapshot.review?' · '+(x.snapshot.review.decision==='approve'?'Approved':'Changes requested')+' · reviewed version '+x.snapshot.review.reviewedVersion:''}{x.action==='decide-request'&&x.snapshot.decision?.reason?' · '+x.snapshot.decision.reason:''}</p></li>)}</ul></details>
