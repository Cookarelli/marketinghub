...
begin
 ...
 for p in select value from jsonb_array_elements(posts) loop
  ...
  if d->>'status'='published' and exists(select 1 from jsonb_array_elements(current_posts) old where old->>'id'=p->>'id' and old->'data'=d) then
   perform private.assert_consignment_approval(d,current_campaign);
  else
